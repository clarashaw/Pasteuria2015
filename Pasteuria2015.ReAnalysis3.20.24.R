...
fst.distances.d$Fstdiv<-fst.distances.d$Fst/(1-fst.distances.d$Fst)

dentfst<-ggplot(fst.distances.d, aes(log(distance+1),Fstdiv))+geom_point()+
  xlab("Ln(distance between lakes (km) + 1)")+theme_bw()+
  theme(axis.text = element_text(color="black"))+ylab(expression(frac(Fst,(1-Fst))))+
  ggtitle("D. dentifera")+theme(plot.title=element_text(face="italic"))
mod<-lm(data=fst.distances.d, Fstdiv~log(distance+1))

d.fst.julian<-inner_join(fst.distances.d, info, by=c("Group1"="species_lake_date"))
d.fst.julian<-select(d.fst.julian, Group1, Group2, Fst, Fstdiv, Julian)
...
ggplot(d.fst.julian, aes(timedif, Fstdiv))+geom_point()
mod<-lm(data=d.fst.julian, Fstdiv~timedif)
...
fst.distances.r$Fstdiv<-fst.distances.r$Fst/(1-fst.distances.r$Fst)

retrofst<-ggplot(fst.distances.r, aes(log(distance+1),Fstdiv))+geom_point()+
  xlab("Ln(distance between lakes (km) + 1)")+theme_bw()+
  theme(axis.text = element_text(color="black"))+ylab(expression(frac(Fst,(1-Fst))))+
  ggtitle("D. retrocurva")+theme(plot.title=element_text(face="italic"))
mod<-lm(data=fst.distances.r, Fstdiv~log(distance+1))
...
r.fst.julian<-select(r.fst.julian, Group1, Group2, Fst,Fstdiv, Julian)
...
ggplot(r.fst.julian, aes(timedif, Fstdiv))+geom_point()
mod<-lm(data=r.fst.julian, Fstdiv~timedif)
...
ggplot(d.fst.julian, aes(timedif, Fstdiv))+geom_point()
mod<-lm(data=d.fst.julian, Fstdiv~timedif) #p=0.06
...
ggplot(r.fst.julian, aes(timedif, Fstdiv))+geom_point()
mod<-lm(data=r.fst.julian, Fstdiv~timedif) #p=0.32
